...
class AskSpec extends TestKit(ActorSystem("AskSpec"))
  ...
  "An authenticator" should {
    AunthenticatorTestSuite(Props[AuthorManagerActor])
  }

  "A pipe authenticator" should {
    AunthenticatorTestSuite(Props[PipedAuthManager])
  }

  def AunthenticatorTestSuite(props: Props) = {
    "fail to authenticate a non-registered user" in {
      val authManager = system.actorOf(props)
      authManager ! Authenticate("arturo", "20906540")
      expectMsg(AuthFailure(AUTH_FAILURE_NOT_FOUND))
    }

    "fail to authenticate if invalid password" in {
      val authManager = system.actorOf(props)
      authManager ! RegisterUser("arturo", "20906540")
      authManager ! Authenticate("arturo", "20906541")
      expectMsg(AuthFailure(AUTH_FAILURE_PASSWORD_INCORRECT))
    }

    "successfully authenticate a register user" in {
      val authManager = system.actorOf(props)
      authManager ! RegisterUser("arturo", "20906540")
      authManager ! Authenticate("arturo", "20906540")
...
object AskSpec{
  ...
  class KvActor extends Actor with ActorLogging{
    ...
    def online(kv: Map[String, String]): Receive= {
      case Read(key) =>
        log.info(s"Try to read the value for the key $key")
  ...
  object AuthManager {
    ...
    val AUTH_FAILURE_SYSTEM = ""
  }
  class AuthorManagerActor extends Actor with ActorLogging {
    import  AuthManager._
    ...
    def handleAuthentication(username: String, password: String) = {
      ...
      // step 4 - handle the future for e.g. with complete
      future.onComplete {
        // Step 5 - most important
        // Never call methods on the actor instance or access on mutable state in ONCOMPLETE
  ...
  class PipedAuthManager extends AuthorManagerActor {
    import AuthManager._
    override def handleAuthentication(username: String, password: String): Unit = {
      ...
      /*
      When the future completes, send the response o the actor ref in arg list
       */
